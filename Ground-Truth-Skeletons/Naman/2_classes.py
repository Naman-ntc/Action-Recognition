...
trainingData = trainingData[:,:,13:21,3]
indices = np.append(np.where(labels==17)[0],np.where(labels==23)[0])
labels = labels[indices]
trainingData = trainingData[indices,:,:,:]

model = Sequential()
model.add(BatchNormalization(input_shape=(300,8,3)))
model.add(Conv2D(32, (3,1), strides=(2,1), padding='same', activation='relu', kernel_initializer='he_normal', kernel_regularizer = regularizer.l2(regParam)))
model.add(BatchNormalization())
model.add(Conv2D(32, (3,1), padding='same', activation='relu', kernel_initializer='he_normal', kernel_regularizer = regularizer.l2(regParam)))
model.add(BatchNormalization())
model.add(MaxPooling2D(pool_size = (2,1)))
model.add(Conv2D(64, (3,1), strides=(2,1), padding='same', activation='relu', kernel_initializer='he_normal', kernel_regularizer = regularizer.l2(regParam)))
model.add(BatchNormalization())
model.add(Conv2D(64, (3,1), padding='same', activation='relu', kernel_initializer='he_normal', kernel_regularizer = regularizer.l2(regParam)))
model.add(BatchNormalization())
model.add(Conv2D(64, (3,1), strides=(2,1), padding='same', activation='relu', kernel_initializer='he_normal', kernel_regularizer = regularizer.l2(regParam)))
model.add(BatchNormalization())
model.add(MaxPooling2D(pool_size=(2,1)))
